...
###=== a) interfaces between the fuel and the pore
  [./interface_from_fuel]
    ...
    master_block = 'fuel'
    paired_block = 'pore'
    new_boundary = master_fuel_interface_pore
  [../]
...
###=== b) interfaces between the pore and the fuel
  [./interface_from_pore]
    ...
    master_block = 'pore'
    paired_block = 'fuel'
    new_boundary = master_pore_interface_fuel
  [../]

###=== c) interface between the fuel and the gap
  [./interface_from_gap_fuel]
    type = SideSetsBetweenSubdomains
    master_block = 'gap'
    paired_block = 'fuel'
    new_boundary = master_gap_interface_fuel
  [../]
...
[InterfaceKernels]
  ...
  [./interface_ld_reaction_pore]
    type = InterfaceForce_ld
    variable = L_dissolve
    neighbor_var = S_dissolve
    boundary = master_pore_interface_fuel
  ...
  [./interface_pureLn_reaction_pore]
 ...
 [./interface_ld_reaction_gap_fuel]
   type = InterfaceForce_ld
   variable = L_dissolve
   neighbor_var = S_dissolve
   boundary = master_gap_interface_fuel
   coupled_l_p = L_precipitate
   coupled_s_p = S_precipitate
 ...
 [./interface_sp_reaction_gap_fuel]
   type = InterfaceForce_sp
   variable = L_dissolve
   neighbor_var = S_precipitate
   boundary = master_gap_interface_fuel
 ...
 [./interface_pureLn_reaction_gap_fuel]
   type = InterfaceForce_lp
   variable = L_precipitate
   neighbor_var = S_dissolve
   boundary = master_gap_interface_fuel
...
###=== Average value on the interface around the pore ======
  ...
  #  type = SideAverageValue
  #  variable = S_dissolve
  #  boundary = master_fuel_interface_pore
  #  outputs = 'exodus'
  #
  #[../]
  #
...
###=== Average value on the interface on the right side of gap ======
  [./L_dissolve_Ave_side_gap_side]
    type = SideAverageValue
    variable = L_dissolve
    boundary = master_gap_end
    outputs = 'exodus console'

  [../]

  [./L_precipitate_Ave_side_gap_side]
    type = SideAverageValue
    variable = L_precipitate
    boundary = master_gap_end
    outputs = 'exodus console'
