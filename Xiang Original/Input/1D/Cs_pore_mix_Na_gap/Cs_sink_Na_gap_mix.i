...
[Mesh]
  type = FileMesh
  file = Export_Cs_pore_mix_Na_gap.e
  #elem_type = QUAD4
  second_order = true
[]

[MeshModifiers]
# Interface between the fuel and the pore
  [./interface_from_s_l]
...
# Interface between the gap (always Na) and the pore
  [./interface_from_g_r]
    type = SideSetsBetweenSubdomains
    master_block = 'gap_r'
    paired_block = 'pore_mix'
...
# interface between the pore and the gap (always Na)
  [./interface_from_pore_r]
    type = SideSetsBetweenSubdomains
    master_block = 'pore_mix'
    paired_block = 'gap_r'
...
[AuxVariables]
  ...
  [./T]
    order = FIRST
    family = LAGRANGE
    block = 'fuel_l pore pore_mix gap_r'
  [../]
...
[AuxKernels]
  ...
  [./Solubility_pore_mix_relate_temperature]
    type = ParsedAux
    variable = Liquid_solubility_Ln
    ...
    # Use the relationship of Ce in liquid Na(50.23%)-Cs
    constant_expressions = '1.084835e10 0.44295 -9.92625 8.61733e-5'
...
# Liquid-state diffusivity in the pore_mix (Na-Cs_filled)
[./Diffusivity_Liquid_pore_mix]
 # Arrhenius equation
  type = ParsedMaterial

  f_name = diffusivity_liquid
  args = T
  constant_names = 'D0_d_Cs Q_d_Cs f_Cs k D0_d_Na Q_d_Na f_Na'

  ## Use the relationship of Ce in liquid Na(50.23%)-Cs
  constant_expressions = '1.56e-4 0.144 0.4977 8.61733e-5 6.658e-4 0.2226 0.5023'
  ...
  # Convert to mcro^2/s
  function = '1e8 * (D0_d_Cs * exp (-Q_d_Cs/k/T) * f_Cs + D0_d_Na * exp (-Q_d_Na/k/T) * f_Na)'      # per minute
  block = 'pore_mix'
[../]
